...
class SpeedCameraUI:
    def __init__(self, master):
        ...
        # Speed Limit
        ttk.Label(self.main_frame, text="Speed Limit (mph):").grid(row=0, column=0, sticky=tk.W, pady=5)
        self.speed_entry = ttk.Entry(self.main_frame)
        self.speed_entry.grid(row=0, column=1, pady=5, padx=10, sticky=tk.EW)
        self.speed_entry.insert(0, "35")  # Default value
        
        # Warning Threshold
        ttk.Label(self.main_frame, text="Warning Threshold (mph):").grid(row=1, column=0, sticky=tk.W, pady=5)
        self.warn_entry = ttk.Entry(self.main_frame)
        self.warn_entry.grid(row=1, column=1, pady=5, padx=10, sticky=tk.EW)
        self.warn_entry.insert(0, "5")  # Default value
        
        # Danger Threshold
        ttk.Label(self.main_frame, text="Danger Threshold (mph):").grid(row=2, column=0, sticky=tk.W, pady=5)
        self.danger_entry = ttk.Entry(self.main_frame)
        self.danger_entry.grid(row=2, column=1, pady=5, padx=10, sticky=tk.EW)
        self.danger_entry.insert(0, "10")  # Default value
    ...
    def validate_and_start(self):
        try:
            speed_limit = float(self.speed_entry.get())
            warn_thresh = float(self.warn_entry.get())
            danger_thresh = float(self.danger_entry.get())
            
            if speed_limit <= 0 or warn_thresh <= 0 or danger_thresh <= 0:
                raise ValueError("Values must be positive")
            if warn_thresh >= danger_thresh:
                raise ValueError("Warning threshold must be less than danger threshold")
            
            self.master.speed_limit = speed_limit
            self.master.warn_thresh = warn_thresh
            self.master.danger_thresh = danger_thresh
            self.master.destroy()
...
class SpeedOverlay:
    ...
    def get_speed_color(self, speed, speed_limit, warn_thresh, danger_thresh):
        if speed >= speed_limit + danger_thresh:
            return self.settings['danger_color']
        elif speed >= speed_limit + warn_thresh:
            return self.settings['warning_color']
        return self.settings['normal_color']

    def draw_overlay(self, frame, speed_data, warn_thresh, danger_thresh):
        height, width = frame.shape[:2]
        overlay = frame.copy()
        cv2.rectangle(overlay, (0, 0), (300, 110), self.settings['bg_color'], -1)
        cv2.addWeighted(overlay, self.settings['bg_alpha'], frame, 
                      1 - self.settings['bg_alpha'], 0, frame)
        ...
        if speed_data:
            speed_text = f"Speed: {speed_data['speed_mph']:.1f} MPH"
            color = self.get_speed_color(
                speed_data['speed_mph'], 
                speed_data['speed_limit'],
                warn_thresh,
                danger_thresh
            )
            cv2.putText(frame, speed_text, self.settings['position'], 
                       cv2.FONT_HERSHEY_SIMPLEX, 1, color, 2)
            
            if speed_data['speed_mph'] > speed_data['speed_limit'] + danger_thresh:
                status = "DANGER"
            elif speed_data['speed_mph'] > speed_data['speed_limit'] + warn_thresh:
                status = "WARNING"
            else:
                status = "NORMAL"
                
            status_pos = (self.settings['position'][0], self.settings['position'][1] + 40)
            cv2.putText(frame, status, status_pos,
                       cv2.FONT_HERSHEY_SIMPLEX, 1, color, 2)
